...
bool libfeature::isSymmetric(InputArray src, double eps) {
    Mat m = src.getMat();
    switch (m.type()) {
...
// libfeature::argsort
//------------------------------------------------------------------------------
Mat libfeature::argsort(InputArray _src, bool ascending) {
    Mat src = _src.getMat();
...
Mat libfeature::histc(InputArray _src, int minVal, int maxVal, bool normed) {
    Mat src = _src.getMat();
    switch (src.type()) {
    case CV_8SC1:
...
//------------------------------------------------------------------------------
// libfeature::sortMatrixColumnsByIndices
//------------------------------------------------------------------------------

void libfeature::sortMatrixColumnsByIndices(InputArray _src, InputArray _indices, OutputArray _dst) {
...
Mat libfeature::sortMatrixColumnsByIndices(InputArray src, InputArray indices) {
    Mat dst;
...
//------------------------------------------------------------------------------
// libfeature::sortMatrixRowsByIndices
//------------------------------------------------------------------------------
void libfeature::sortMatrixRowsByIndices(InputArray _src, InputArray _indices, OutputArray _dst) {
    if(_indices.getMat().type() != CV_32SC1) {
    	string error_message = format("cv::sortRowsByIndices only works on integer indices! Expected: %d. Given: %d.", CV_32SC1, _indices.getMat().type());
    	CV_Error(CV_StsBadArg, error_message);
    }
...
Mat libfeature::sortMatrixRowsByIndices(InputArray src, InputArray indices) {
   Mat dst;
...
//------------------------------------------------------------------------------
// libfeature::asRowMatrix
//------------------------------------------------------------------------------
Mat libfeature::asRowMatrix(InputArrayOfArrays src, int rtype, double alpha, double beta) {
...
//------------------------------------------------------------------------------
// libfeature::asColumnMatrix
//------------------------------------------------------------------------------
Mat libfeature::asColumnMatrix(InputArrayOfArrays src, int rtype, double alpha, double beta) {
    // make sure the input data is a vector of matrices or vector of vector
    if(src.kind() != _InputArray::STD_VECTOR_MAT && src.kind() != _InputArray::STD_VECTOR_VECTOR) {
        CV_Error(CV_StsBadArg, "The data is expected as InputArray::STD_VECTOR_MAT (a std::vector<Mat>) or _InputArray::STD_VECTOR_VECTOR (a std::vector< vector<...> >).");
    }
