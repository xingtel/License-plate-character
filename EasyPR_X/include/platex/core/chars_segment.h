...
class CCharsSegment {
 public:
  ...
  int charsSegment9(Mat input, std::vector<Mat>& resultVec, Color color = BLUE);

  void GetNewRoi(Color plateType, Mat input,Mat& newRoi);
  bool verifyCharSizes(Mat r, int h = 40);
  ...
  int GetSpecificRect(const std::vector<Rect>& vecRect,Color color = BLUE);
