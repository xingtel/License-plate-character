...
class CPlateLocate {
 public:
  ...
  int sobelFrtSearchT(const Mat& src, std::vector<Rect_<float>>& outRects);
  ...
  int plateMserLocateT(Mat src, vector<CPlate> &candPlates, int img_index);

  int colorSearch(const Mat& src, const Color r, Mat& out,
                  std::vector<RotatedRect>& outRects);

  int colorSearchT(const Mat& src, const Color r, std::vector<cv::Rect>& outRects);
