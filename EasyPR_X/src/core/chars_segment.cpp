#include "platex/core/chars_segment.h"
...
void CCharsSegment::GetNewRoi(Color plateType,Mat auxRoi,Mat& newRoi)
{
	if (BLUE == plateType) {
...
bool CCharsSegment::verifyCharSizes(Mat r,int h) {
  // Char sizes 45x90
  float aspect = 45.0f / 90.0f;
  float charAspect = (float)r.cols / (float)r.rows;
  float error = 0.7f;
  float minHeight = h*0.3;
  float maxHeight = h*1.0;
...
int CCharsSegment::charsSegment9(Mat input, std::vector<Mat>& resultVec, Color color ){
	...
	spatial_ostu(img_threshold, 8, 3, plateType);
	...
	int cW = (color == GREEN) ? (input.cols / 9) : (input.cols / 8);
	while (itc != contours.end()) {
		Rect mr = boundingRect(Mat(*itc));
		if (mr.x + mr.width / 2 > cW)
		{
			Mat auxRoi(img_threshold, mr);

			if (verifyCharSizes(auxRoi, input.rows))
			{
	...
	specIndex = GetSpecificRect(sortedRect, color);
	...
	{
		sortedRect.erase(sortedRect.begin());
	}
	chineseRect = sortedRect[0];
	chineseRect.x = chineseRect.x - chineseRect.width*1.2;
	if (chineseRect.x < 0) chineseRect.x = 0;
	chineseRect.width = sortedRect[0].x - chineseRect.x;
	sortedRect.insert(sortedRect.begin(),chineseRect);
	vector<Rect> newSortedRect;
	//newSortedRect.push_back(chineseRect);
	//RebuildRect(sortedRect, newSortedRect, specIndex);

	newSortedRect = sortedRect;
	...
	for (size_t i = 0; i < newSortedRect.size(); i++) {
		...
		//else 
		{
			GetNewRoi(plateType, auxRoi,newRoi);
		}
...
int CCharsSegment::GetSpecificRect(const vector<Rect>& vecRect, Color color) {
  ...
  for (size_t i = 0; i < vecRect.size(); i++) {
	...
	char szBuf[128];
	sprintf(szBuf,"%d", m_theMatWidth);
    // use known knowledage to find the specific character
    // position in 1/7 and 2/7
    if ((mr.width > maxWidth * 0.5 && mr.height > maxHeight * 0.6) &&
        (midx < int(m_theMatWidth * 2 / wCount)  &&
         midx > int(m_theMatWidth / wCount) * 1)) {
      specIndex = i;
    }
  }
...
int CCharsSegment::RebuildRect(const vector<Rect>& vecRect,
  ...
  int count = 6;
  for (size_t i = specIndex; i < vecRect.size() ; ++i) {
