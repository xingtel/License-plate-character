...
void getLBPFeatures(const Mat& image, Mat& features) {
  ...
  Mat lbpimage;
  lbpimage = libfeature::olbp(grayImage);
  Mat lbp_hist = libfeature::spatial_histogram(lbpimage, 32, 4, 4);

  features = lbp_hist;
}

Mat charFeatures(Mat in, int sizeData, bool bCutC) {
  const int VERTICAL = 0;
  const int HORIZONTAL = 1;
...
void getLBPplusHistFeatures(const Mat& image, Mat& features) {
  ...
  Mat lbpimage;
  lbpimage = libfeature::olbp(grayImage);
  Mat lbp_hist = libfeature::spatial_histogram(lbpimage, 64, 8, 4);
