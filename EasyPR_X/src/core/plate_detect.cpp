#include "platex/core/plate_detect.h"
...
namespace platex {
    ...
    bool showDetectArea, int img_index) 
  {
	...
	DrawThePicPoint(gShowWnd, src, 5, 180);

        if ( type == PR_DETECT_SOBEL)
		{
			//sobel
			m_plateLocate->plateSobelLocate(src, sobel_Plates, img_index);
        }
    
       if ( type == PR_DETECT_COLOR)
	   {
		   m_plateLocate->plateColorLocate(src, color_Plates, img_index);
       }

        if ( type == PR_DETECT_CMSER) 
		{
			 m_plateLocate->plateMserLocate(src, mser_Plates, img_index);
        }

		if (type == (PR_DETECT_CMSER | PR_DETECT_SOBEL))
		{
	
				//Sobel + mser
				std::vector<cv::Rect_<float>> plateRs;
				m_plateLocate->sobelFrtSearchT(src, plateRs);

				for (int k = 0; k < plateRs.size(); ++k)
				{
					std::vector<CPlate> _Plates;
					m_plateLocate->plateMserLocateT(src(plateRs[k]), _Plates, img_index);
		...
		if (type == (PR_DETECT_CMSER | PR_DETECT_COLOR))
		{

			//Sobel + mser
			std::vector<cv::Rect> plateRs;
			m_plateLocate->colorSearchT(src,BLUE, plateRs);

			std::vector<cv::Rect> plateYs;
			m_plateLocate->colorSearchT(src, YELLOW, plateYs);
			for (int k = 0; k < plateYs.size(); ++k)
			{
				plateRs.push_back(plateYs[k]);
			}

			for (int k = 0; k < plateRs.size(); ++k)
			{
				std::vector<CPlate> _Plates;
				m_plateLocate->plateMserLocate(src(plateRs[k]).clone(), _Plates, img_index);
