#include "platex/core/chars_recognise.h"
    ...
    for (int j = 0; j < num; j++)
    {
      ...
      if (j == 0) {
		...
		std::string strReco;
		 ...
		 plateX_recoMat(charMat, strReco,true); // CharsIdentify::instance()->identifyChinese(charMat, maxVal, judge);
        
		 plateLicense.append(strReco);
      }
      else {
        isChinses = false;
		...
		if (j == 1)
		{
			plateX_recoCh(charMat, strReco, false);
		}
		else
		{
			plateX_recoMat(charMat, strReco, false);
		}
