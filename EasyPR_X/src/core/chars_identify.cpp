...
namespace platex {
  ...
  std::pair<std::string, std::string> CharsIdentify::identifyChinese(cv::Mat input, float& out, bool& isChinese, bool bCutC)
  {
  ...
  std::pair<std::string, std::string> CharsIdentify::identify(cv::Mat input, bool isChinese,bool bCutC) {
    cv::Mat feature = charFeatures(input, kPredictSize, bCutC);
    float maxVal = -2;
    auto index = static_cast<int>(classify(feature, maxVal, isChinese));
    if (index < kCharactersNumber) {
