...
#include "platex/core/plate.hpp"
#include "platex/core/chars_identify.h"
...
#define PLATE_CHAR_COUNT 8
	// Chinese plate has max 7 characters.
const int char_max_count = PLATE_CHAR_COUNT;
...
Mat colorMatch(const Mat &src, Mat &match, const Color r,
 ...
 //H range of Green 绿色新能源车牌
  const int min_green = 68;  
  const int max_green = 82;  
...
bool clearLiuDing(Mat &img) {
  ...
  for (int i = 0; i < img.rows; i++) {
	...
	fCountW.push_back(maxW*1.0/ img.cols);
    jump.at<float>(i) = (float) jumpCount;
  }
  ...
  for (int i = 0; i < img.rows; i++)
  {
    if (jump.at<float>(i) <= x || fCountW[i] >= 0.26)
	{
      for (int j = 0; j < img.cols; j++) 
	  {
        img.at<char>(i, j) = 0;
...
void affine(const Mat &in, Mat &out,const double slope) {
	...
	if (slope > 0) {
		...
		float a = 0.8;
		dstTri[0] = Point2f(xiff*a , 0);
		dstTri[1] = Point2f(width - 1 - xiff*a, 0);
		dstTri[2] = Point2f(xiff*a, height - 1);
	}
	else {
		...
		float a = 0.2;
		dstTri[0] = Point2f(xiff*a, 0);
		dstTri[1] = Point2f(width - 1 - xiff + xiff*a, 0);
		dstTri[2] = Point2f(xiff*a, height - 1);
	}
	...
	Mat affine_mat;
	affine_mat.create((int)height, (int)width, in.type());
...
//对车牌进行纠偏处理
void rectifyPlateEasyPR(Mat& tmpSrc, std::vector<cv::Rect>& outRect,bool& bNeedCSeg)
{
	Rect minR = outRect[0];
	Rect maxR = outRect[outRect.size() - 1];

	int X = maxR.x - minR.x-minR.width;
	int Y = maxR.y - minR.y;
	...
	if (angle < -5.5 || angle > 5.5)
	{
		...
		{
			pts.push_back(Point(outRect[i].x, outRect[i].y));
		}

		RotateMatAndPts(pts, tmpSrc, angle, Y);
		for (int i = 0; i < outRect.size(); ++i)
		{
			outRect[i].x = pts[i].x;
			outRect[i].y = pts[i].y;
		...
		Mat img_crop;
		getRectSubPix(tmpSrc, Size(tmpSrc.cols, outRect[0].height*1.36),
			Point(tmpSrc.cols / 2, outRect[0].y + outRect[0].height / 2), img_crop);

		int tY = (outRect[0].y + outRect[0].height / 2 - outRect[0].height*1.36 / 2);
		...
		Mat outSlope;
		affine(img_crop, outSlope, tan(asin((Y / R))));
		...
		{
			//纠正位置
			outRect[k].x += tan(asin((Y / R))) * img_crop.rows*0.8;
			if (outRect[k].x < 0) outRect[k].x = 0;
			if (outRect[k].x + outRect[k].width>= outSlope.cols)
				outRect[k].width = outSlope.cols - 1-outRect[k].x;

			if (outRect[k].y < 0) outRect[k].y = 0;
			if (outRect[k].y + outRect[k].height >= outSlope.rows)
				outRect[k].height = outSlope.rows - outRect[k].y-1;
...
void CorrectBd_Blue(Mat& src, std::vector<cv::Rect>& inRect, Color color, int& outL, int& outR)
{
	Rect minR = inRect[0];
	Rect maxR = inRect[inRect.size() - 1];

	int L1 = inRect[0].x - inRect[0].width*1.3;
	int R1 = maxR.x + maxR.width + maxR.width*1.3;
	if (L1 < 0) L1 = 0;
	if (R1 >= src.cols) R1 = src.cols - 1;
	int ty = min(minR.y, maxR.y);
	int by = max(minR.y + minR.height, maxR.y + maxR.height);
	outL = inRect[2].x;

	for (int j = L1; j < inRect[2].x; ++j)
	{
		int iCount = 0;
		for (int i = ty; i < by; ++i)
		{	
			...
			if ((src.data[step + j * 3 + 0] > src.data[step + j * 3 + 1]+12
				&& src.data[step + j * 3 + 1] > src.data[step + j * 3 + 2] + 10)
				|| (src.data[step + j * 3 + 0] > src.data[step + j * 3 + 1] + 30
					&& src.data[step + j * 3 + 0] > src.data[step + j * 3 + 2] + 30))
			{
				int step1 = src.step[0] * (inRect[2].y + inRect[2].height / 2);

				int iSimCount = 0;
				//不仅是绿色要和中间颜色相近
				for (int k = inRect[2].x; k < inRect[3].x + inRect[3].width; ++k)
				{
					int a1 = fabs(src.data[step + j * 3 + 0] - src.data[step1 + k * 3 + 0]);
					int a2 = fabs(src.data[step + j * 3 + 1] - src.data[step1 + k * 3 + 1]);
					int a3 = fabs(src.data[step + j * 3 + 2] - src.data[step1 + k * 3 + 2]);
					if (a1 < 15 && a2 < 15 && a3 < 15)
					{
						
						sprintf(szBuf, "%d %d %d %d %d %d", src.data[step + j * 3 + 0], src.data[step + j * 3 + 1], src.data[step + j * 3 + 2],
							src.data[step1 + k * 3 + 0], src.data[step1 + k * 3 + 1], src.data[step1 + k * 3 + 2]);
						iSimCount++;
	...
	for (int j = R1; j > maxR.x; --j)
	{
		...
		for (int i = ty; i < by; ++i)
		{
			...
			if (src.data[step + j * 3 + 0] > src.data[step + j * 3 + 1] + 12
				&& src.data[step + j * 3 + 1] > src.data[step + j * 3 + 2] + 10
				|| (src.data[step + j * 3 + 0] > src.data[step + j * 3 + 1] + 50
					&& src.data[step + j * 3 + 0] > src.data[step + j * 3 + 2] + 30))
			{
				int step1 = src.step[0] * (inRect[2].y + inRect[2].height / 2);
				//不仅是绿色要和中间颜色相近
				for (int k = inRect[2].x; k < inRect[3].x + inRect[3].width; ++k)
				{
					...
					if (a1 + a2 + a3 < 30)
					{
						...
						sprintf(szBuf, "%d %d %d %d %d %d", src.data[step + j * 3 + 0], src.data[step + j * 3 + 2], src.data[step + j * 3 + 2],
							src.data[step1 + k * 3 + 0], src.data[step1 + k * 3 + 1], src.data[step1 + k * 3 + 2]);
						iCount++;
						break;
					}
	...
	{
		//是否能够在左边添加一个框
		Rect tmpRect = inRect[0];
		if (tmpRect.x + tmpRect.width < outL)
		{
			inRect.erase(inRect.begin());
		}
		else
		{
			inRect[0].x = outL;
	...
	if (fabs(inRect[0].x - inRect[1].x) < 4)
	{
		inRect.erase(inRect.begin());
	}
	...
	//对方框进行纠正
	if (outR < maxR.x + maxR.width*0.2)
	{
		//inRect.erase(inRect.end()-1);
	}

}

void CorrectBd_Yellow(Mat& src, std::vector<cv::Rect>& inRect, Color color, int& outL, int& outR)
{
	Rect minR = inRect[1];
	Rect maxR = inRect[inRect.size() - 1];
	...
	for (int j = L1; j < inRect[2].x; ++j)
	{
		...
		{
			int step = src.step[0] * i;
			if (src.data[step + j * 3 + 1] > src.data[step + j * 3 + 0]+30
				&& src.data[step + j * 3 + 2] > src.data[step + j * 3 + 0] + 30)
			{
				iCount++;
			}
		}
...
void CorrectBd_Green(Mat& src, std::vector<cv::Rect>& inRect, Color color, int& outL, int& outR)
{
		Rect minR = inRect[0];
		Rect maxR = inRect[inRect.size() - 1];
		...
		int by = max(minR.y + minR.height, maxR.y + maxR.height);
		outL = 0;
		for (int j = L1; j < minR.x+minR.width; ++j)
		{
			char szBuf[256];
			int iCount = 0;
			for (int i = ty; i < by; ++i)
			{
				int step = src.step[0] * i;
				if (src.data[step + j * 3 + 1] > src.data[step + j * 3 + 0]+10
					&& src.data[step + j * 3 + 1] > src.data[step + j * 3 + 2]+10
					&& src.data[step + j * 3 + 1] > 90)
				{
					iCount++;
					sprintf(szBuf, "%d %d %d %d %d %d", src.data[step + j * 3 + 0], src.data[step + j * 3 + 1], src.data[step + j * 3 + 2],
						src.data[step + j * 3 + 0], src.data[step + j * 3 + 1], src.data[step + j * 3 + 2]);
				}
			}
			if (iCount >= 4)
			{
		...
		for (int j = R1; j >= maxR.x; --j)
		{
			...
			for (int i = ty; i < by; ++i)
			{
				...
				{
					sprintf(szBuf, "%d %d %d %d %d %d", src.data[step + j * 3 + 0], src.data[step + j * 3 + 1], src.data[step + j * 3 + 2],
						src.data[step + j * 3 + 0], src.data[step + j * 3 + 1], j);
				}
			
				if (src.data[step + j * 3 + 1] > src.data[step + j * 3 + 0] + 5
					&& src.data[step + j * 3 + 1] > src.data[step + j * 3 + 2] + 5
					&& src.data[step + j * 3 + 1] > 90)
				{
		...
		for (int k = 0; k < inRect.size(); ++k)
		{
			//rectangle(src,inRect[k],Scalar(255,0,255),2);
		}
		//imshow("src",src);
		//waitKey();
		...
		{
			Rect tmpR = inRect[k];
			if (tmpR.x + tmpR.width/2 < outL)
			{
				inRect.erase(inRect.begin());
			}
			else
			{
				if (outL < tmpR.x - tmpR.width*0.65)
				{
					//添加一个
					Rect newR = tmpR;
					newR.x = outL;
					newR.width = tmpR.x - outL;
					inRect.insert(inRect.begin(),newR);
				}
				else
				{
					inRect[k].x = outL;
					inRect[k].width = inRect[k + 1].x - inRect[k].x;
				}
...
//旋转车牌进行纠正，便可ok
void GetLeftRightPos(Mat& src, vector<cv::Rect>& inRect, Color color)
{

	//有空格的，产生框
	for (int i = 0; i < inRect.size()-1; ++i)
	{
		Rect ri = inRect[i]; Rect ri1 = inRect[i + 1];
		if (ri1.x - ri.x - ri.width >= (ri1.width + ri.width)*0.3)
		{
			inRect.insert(inRect.begin() + i + 1, cv::Rect(ri.x + ri.width, ri.y,
				ri1.x - ri.x - ri.width, ri.height));
		}
	}
	//纠正偏
	bool bNeedCSeg = false;
	rectifyPlateEasyPR(src, inRect, bNeedCSeg);
	
	Mat showTmp = src.clone();
	for (int k = 0; k < inRect.size(); ++k)
	{
		rectangle(showTmp, inRect[k], Scalar(0, 0, 255), 1);
	}
	//imshow("showtmp", showTmp);
	//waitKey();

	int outL, outR;
	if (color == GREEN)
	{
		CorrectBd_Green(src, inRect, color, outL, outR);
	}
	else if(color == BLUE)
	{
		CorrectBd_Blue(src, inRect, color, outL, outR);
	}
	else if (color == YELLOW)
	{
		CorrectBd_Yellow(src, inRect, color, outL, outR);
	}
	//判断右边
	bNeedCSeg = true;
}

bool IsBlackColor(const Mat src)
{
	int iCount = 0;
	for (int i = 0; i < src.rows; ++i)
	{
		...
		{//BGR
			if (src.data[step + jump + 0] < 42
				&& src.data[step + jump + 1] < 42
				&& src.data[step + jump + 2] < 42)
			{
				iCount++;
			}
	...
	float fScore = iCount*1.0 / (src.cols*src.rows);
	char szBuf[256];
	sprintf(szBuf, "%d %d %d  %f", iCount, src.cols, src.rows, fScore);
...
bool IsGreenColor(const Mat src)
{
	int iCount = 0;
	for (int i = 0; i < src.rows; ++i)
	{
		...
		for (int j = 0; j < src.cols; ++j)
		{//BGR
			if (src.data[step + jump + 1] > src.data[step + jump + 0]+5
				&& src.data[step + jump + 1] > src.data[step + jump + 2]+5)
			{
				iCount++;
			}
...
bool verifyRotatedPlateSizes(RotatedRect mr, bool showDebug) {
  ...
  // Real car plate size: 136 * 32, aspect 4
  float aspect = 3.75*PLATE_CHAR_COUNT/7 ;
...
void reFoundAndCombineRect(std::vector<CCharacter>& mserCharacter, float min_thresh, float max_thresh, 
  ...
  for (; mserCharacter_i + 1 < mserCharacter.size(); mserCharacter_i++) {
    ...
    if (ratio > max_thresh) {
      ...
      if (1) {
        cv::rectangle(result, rect_add, Scalar(0, 255, 255));
      }
    }
...
bool strongSeedConsfirstplate(platex::CPlate&  plate, std::vector<CCharacter>& searchCandidate ,
	const cv::Mat& image,cv::Mat& result,const Color& the_color)
{
		...
		if (mserCharacter.size() < char_max_count)
		{
			...
			if (1) {
				...
				if (1 /* && showDebug*/) {
					...
					char szBuf[128];
					sprintf(szBuf, "**************char  %d %s %f \n", leftIsChinese, label.c_str(), maxVal);
					TRACE0(szBuf);
				}
			}
		...
		if (verifyRotatedPlateSizes(platePos)) {
			...
			std::vector<CCharacter> plateChars = plate.getCopyOfMserCharacters();
			for (int k = 0; k < mserCharacter.size(); ++k)
			{
				plateChars.push_back(mserCharacter[k]);
			}
			plate.setMserCharacter(plateChars);
...
void DomserCharMatch(const Mat &image, std::vector<Mat> &match, std::vector<std::vector<std::vector<Point>>>& all_contours,
...
std::vector<CPlate>& out_plateVec_blue,
	...
	int color_index, Color the_color)
{
	...
	for (size_t index = 0; index < size; index++) {
		...
		if (usePlateMser) {
			...
			if (verifyRotatedPlateSizes(rrect)) {
				...
				out_plateRRect_blue.push_back(rrect);
	...
	for (auto charGroup : charGroupVec) 
	{
		...
		if (points.size() >= 2 && ratio_maxrect >= 0.3) {
			...
			int mindist = PLATE_CHAR_COUNT * maxrect.width;
	...
	{
		if (strongSeedConsfirstplate(plateVec[k],searchCandidate, image,result, the_color))
		{
			out_plateVec_blue.push_back(plateVec[k]);

			//DrawThePicPoint(gShowWnd, src, 5, 110);

			if (0/*&& showDebug*/)
			{
				imshow("result3", result);
				waitKey(0);
				destroyWindow("result3");
			}
		}
...
//! use verify size to first generate char candidates
void mserCharMatchT(const Mat &src, std::vector<Mat> &match, 
	std::vector<CPlate>& out_plateVec_blue, std::vector<CPlate>& out_plateVec_yellow,
  bool usePlateMser, std::vector<RotatedRect>& out_plateRRect_blue, std::vector<RotatedRect>& out_plateRRect_yellow)
{
  Mat image = src;
  ...
  // color_index = 1 : mser+, detect dark characters, which is in yellow plate.
    ...
    Color the_color = flags.at(0);
	DomserCharMatch(image, match, all_contours, all_boxes, out_plateVec_blue, 
		usePlateMser, out_plateRRect_blue, 0, the_color);

	the_color = flags.at(1);
	DomserCharMatch(image, match, all_contours, all_boxes, out_plateVec_yellow,
		usePlateMser, out_plateRRect_yellow, 1, the_color);
