...
#include "platex/core/plate_locate.h"
#include "platex/core/core_func.h"

#include "platex/util/util.h"
...
//! mser search method
int CPlateLocate::mserSearchT(const Mat &src,
  vector<vector<CPlate>>& out_plateVec, bool usePlateMser, vector<vector<RotatedRect>>& out_plateRRect) {
  vector<Mat> match_grey;
  ...
  mserCharMatchT(src, match_grey, plateVec_blue, plateVec_yellow, usePlateMser, plateRRect_blue, plateRRect_yellow);
...
int CPlateLocate::colorSearchT(const Mat& src, const Color r, std::vector<cv::Rect>& outRects)
{
	Mat match_grey;
	...
	localFangchaContrasMy lcG;
	vector<cv::Rect> platesR;
	lcG.GetPlateRects(src_threshold, platesR);
	outRects = platesR;

	for (int j = 0; j < platesR.size(); ++j)
	{
		//rectangle(src_threshold, platesR[j], Scalar(100));
		
	}

	char szBuf[126];
	sprintf(szBuf,"color%d", r);
	cv::imshow(szBuf, src_threshold);
	return 1;
}
...
int CPlateLocate::sobelFrtSearchT(const Mat& src, std::vector<Rect_<float>>& outRects)
{
	Mat src_threshold;
	...
	localFangchaContrasMy lcG;
	vector<cv::Rect> platesR;
	lcG.GetPlateRectsNew(src_threshold, platesR);
	for (int j = 0; j < platesR.size(); ++j)
	{
		rectangle(src_threshold, platesR[j], Scalar(120),3);
		outRects.push_back(cv::Rect_<double>(platesR[j]));

		char szBuf[256];
		sprintf(szBuf, "%d", j);
		cv::putText(src_threshold, szBuf, cv::Point(platesR[j].x, platesR[j].y), 0, 0.6, Scalar(128), 2,cv::LINE_AA);
	}
	
	//cv::imshow("fisrtSobel", src_threshold);

	DrawThePicPoint(gShowWnd, src_threshold, 705, 180);
	return 1;
}
...
int CPlateLocate::sobelSecSearchPart(Mat &bound, Point2f refpoint,
  ...
  if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
    ...
    utils::imwrite("/repaireimg1.jpg", bound_threshold);
    ...
    utils::imwrite("/repaireimg2.jpg", bound_threshold);
  }
...
int CPlateLocate::deskewMser(const Mat &src,
	...
	vector<CPlate> &outPlates, bool useDeteleArea, Color color) 
{
	...
	for (size_t i = 0; i < inRects.size(); i++) {
		...
		if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
			...
			if (deskew_mat.cols * 1.0 / deskew_mat.rows > 1.19 &&
				deskew_mat.cols * 1.0 / deskew_mat.rows < 7.0)
			{

				plate_mat = deskew_mat;
				CPlate plate;
				plate.setPlatePos(roi_rect);
				plate.setPlateMat(plate_mat);
				plate.m_boundRect = safeBoundRect;
...
//! MSER plate locate
int CPlateLocate::plateMserLocateT(Mat src, vector<CPlate> &candPlates, int img_index)
{
	...
	for (size_t i = 0; i < channelImages.size(); ++i) {
		...
		Mat image = channelImages.at(i);

		// vector<RotatedRect> rects;
		mserSearchT(image, platesVec, usePlateMser, plateRRectsVec);
		...
		for (size_t j = 0; j < flags.size(); j++) {
			vector<CPlate>& plates = platesVec.at(j);
			{
				...
				{
					plates[k].setPlateMat(src);
					candPlates.push_back(plates[k]);
				}
			}
